...
u1_e_fwd_data= np.load('Velocity_1_ensemble_forward_obs.npy')
u2_e_fwd_data= np.load('Velocity_2_ensemble_forward_obs.npy')
...
u1_e_obs_fwd_obs = np.transpose(u1_e_fwd_data, (1,0,2))# use while plotiing against N_obs
u2_e_obs_fwd_obs = np.transpose(u2_e_fwd_data, (1,0,2))# use while plotiing against N_obs
...
u1_e_obs_fwd_spatial = np.transpose(u1_e_fwd_data, (1,2,0)) # use while plotiing against X
u2_e_obs_fwd_spatial = np.transpose(u2_e_fwd_data, (1,2,0)) # use while plotiing against X
...
ax1.plot(u1_e_obs_fwd_spatial[Obs_N,:,:], 'g-')
...
ax2.plot(u2_e_obs_fwd_spatial[Obs_N,:,:], 'g-')
